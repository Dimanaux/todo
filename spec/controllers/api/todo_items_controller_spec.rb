...
describe Api::TodoItemsController do
  ...
  describe 'GET #index' do
    context 'when viewing my todo list' do
      let(:list) { create(:todo_list, user: current_user) }
      let(:item) { create(:todo_item, user: current_user, todo_list: todo_list) }

      it 'returns a success response' do
        get :index, params: { todo_list_id: list.to_param }
        expect(response).to be_successful
      end

      it 'returns all todo list items' do
        get :index, params: { todo_list_id: list.to_param }
        expect(response_body.map { |h| h['id'].to_i }.sort).to eq(list.todo_item_ids.sort)
      end
    end
  ...
  describe 'GET #show' do
    it 'returns a success response' do
      ...
      get :show, params: {id: todo_item.to_param}
      expect(response).to be_successful
    end
  end
